<template>
  <q-form @submit="sendResetPassword()">
...
<script>
import { resetPassword } from 'src/services/auth/auth-api.js';
...
export default {
  ...
  methods: {
    sendResetPassword: async function () {
      this.loading = true;
      try {
        const { data } = await resetPassword(this.formData, this.$route.params.token);
        this.$q.notify({
          type: 'positive',
